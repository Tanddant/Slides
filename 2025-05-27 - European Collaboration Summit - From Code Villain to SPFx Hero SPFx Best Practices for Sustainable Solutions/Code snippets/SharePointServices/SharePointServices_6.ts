...
export class SharePointServices6 implements ISharePointServices {
    private SPFI: SPFI;
    ...
    public async GetAllSpeakersByCountry(country: string): Promise<ISpeaker[]> {
        return this.SPFI.web.lists.getByTitle(ListNames.Speakers).items.filter<ISpeaker>(
            f => f.text("country").equals(country)
        )();
    }
}
